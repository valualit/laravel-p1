	...
	<div class="tab-pane fade show active" id="default" role="tabpanel" aria-labelledby="default-tab">
		<div class="form-group">
			...
			<div class="input-group">
				...
				<input type="text" class="form-control" id="name" name="user[name]" placeholder="{{ __('Имя пользователя/Login') }}" value="{{$user->name}}" />
			</div>
		...
		<div class="form-group">
			...
			<div class="input-group">
				...
				<input type="text" class="form-control" id="email" name="user[email]" value="{{$user->email}}" placeholder="{{ __('E-Mail') }}" />
			</div>
		...
		$( document ).ready(function() {
			$('#email').keyup(function(){isUserEmail({{$user->id}});}).change(function(){isUserEmail({{$user->id}});}).click(function(){isUserEmail({{$user->id}});});
		}); </script>
		<div class="form-group">
			...
			<div class="input-group">
				...
				<input type="text" class="form-control" id="phone" value="{{$user->phone}}" name="user[phone]" placeholder="{{ __('Телефон') }}" />
			</div>
		</div> 
		<div class="form-group">
		...
		<div class="form-group">
			...
			<select class="form-control" id="roles" name="user[roles]">
				...
				<option value="{{$role->id}}"{{$role->id==$user->roles?' selected':null}}>{{$role->name}}</option>
				@endforeach
			</select>
		</div> 
