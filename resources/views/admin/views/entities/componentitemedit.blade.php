	...
	<div class="tab-pane fade show active" id="default" role="tabpanel" aria-labelledby="default-tab">
		<div class="form-group">
			...
			<div class="input-group">
				<input type="text" class="form-control" id="name" name="section[name]" placeholder="{{$ComponentSettings['namerow']??'Заголовок'}}" value="{{$item->name}}" />
			</div>		
		</div>
		<div class="form-group">
			<label for="text">{{ __('Текст') }}</label>
			{!! EntitiesFields::TypesList()["textarea wysiwyg"]["html"](__('Текст'),"text","section[text]",null, $item->text) !!}	
		</div> 
		<div class="form-group">
			<label for="created_at">{{ __('Дата создания') }}</label>
			{!! EntitiesFields::TypesList()["input-date"]["html"](__('Дата создания'),"created_at","section[created_at]",null, $item->created_at) !!}	
		</div> 
		<div class="form-group">
			<label for="user">{{ __('ID автора') }}</label>
			<div class="input-group">
				<input type="text" class="form-control" id="user" name="section[user]" placeholder="{{ __('ID автора') }}"  value="{{$item->user}}" />
			</div>		
		</div>
		<div class="form-group">
			<label for="parent">{{ __('ID родительского елемента') }}</label>
			<div class="input-group">
				<input type="text" class="form-control" id="parent" name="section[parent]" placeholder="{{ __('ID автора') }}"  value="{{$item->parent}}" />
			</div>		
		</div>
